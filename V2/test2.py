...
parser.add_argument("--layer", default="L1", type=str, help="the feature layer we are testing")
...
testset = {'LR':testset}
net, im_input = LayerExtractor.forward(dataset=testset,mode='LR',layer=opt.layer)
im_input = torch.squeeze(im_input.data).numpy()

im_input = im_input/255.
im_input = Variable(torch.unsqueeze(torch.from_numpy(im_input).float(),dim=0))
...
im_h_y=LayerExtractor.inverse(forward_result=Variable(forward_result),net=net,layer =opt.layer)

...
def PickleHelper(action,input,fileName,folder = '.'):
    import pickle
    import os
    if action == 'dump':
...
def meanShiftHelper(input,n_clusters = None,meanShifts = None, n_features=4):
    from sklearn.cluster import estimate_bandwidth
    if len(input.shape) == 4:
        sampleNum = input.shape[0]; featureNum = input.shape[1]* input.shape[2]* input.shape[3]
        input = np.reshape(np.transpose(input,(0,2,3,1)),(sampleNum,featureNum))
        

    half = int((input.shape[1] - 1)/2)
    input = np.concatenate((input[:,:n_features+1],input[:,half+1:half+1+n_features]),axis=1)

#    BW = 93.95470252088799
    from sklearn.cluster import MeanShift
    meanShifts = {}
    labels = np.zeros((input.shape[0],n_features*2+1))
    if not meanShifts:
        tic = time.clock()
        for i in range(n_features*2+1):
            BW = estimate_bandwidth(input[:,[i]],n_jobs=-2,quantile=0.01)
            print('bandwidth is {}'.format(BW))
            meanShifts[i] = MeanShift(n_jobs=-2,bin_seeding = True, cluster_all = False,bandwidth=BW)
            labels[:,i] = meanShifts[i].fit_predict(input[:,[i]])
        print('this kmeans time is: ',time.clock()-tic)
        return meanShifts,labels
    else:
        for i in range(n_features*2+1):
            labels[:,i] = meanShifts[i].predict(input[:,[i]])
        return None,labels

    
def countHist(meanShifts,isPrint = True):
    iter_comb = []
    n_clusters = 1
    import itertools
    for k,meanShift in meanShifts.items():
        if k == 0: continue
        n_cluster = meanShift.cluster_centers_.shape[0]
        if isPrint:
            for i in range(-1,n_cluster):
                print('feature {}: cluster {} has {} samples'.format(k,i,np.sum(meanShift.labels_==i)))
        n_cluster = 2
        iter_comb.append(list(range(0,n_cluster)))
        n_clusters *=n_cluster
    return itertools.product(*iter_comb),n_clusters

def checkSingleCluster(clusterI,imgBatch_LR,imgBatch_HR,labels):
    ...
    count = 0  
    for i in range(size):
#        if ((len(labels[i])==1 and labels[i]==clusterI) or (len(labels[i])!=1 and np.logical_and(labels[i]==clusterI))) and np.random.uniform()<0.2:
        if (labels[i]==clusterI).all() and np.random.uniform()<0.2:
            ax = plt.subplot(1,12,1+count)
            tmp = np.clip(imgBatch_LR[i],0,255)
            ax.imshow(tmp[0],cmap='gray')
            ax.set_title("LR")
            
#            ax = plt.subplot(1,12,count+7)
...
def checkCluster(cluster_Iter,n_clusters,imgBatch_LR,imgBatch_HR,labels):
    import matplotlib.pyplot as plt
    plt.figure(figsize=(25,100))
    
    size = imgBatch_LR.shape[0]
    
    clusterCount = -1
    
    for clusterI in cluster_Iter:
        clusterCount +=1
        count = 0  
        clusterSize = np.sum(np.sum(labels==clusterI,axis=1)==labels.shape[1])
        print('cluster {} has {}'.format(clusterI,clusterSize))
        for i in range(size):
            if (labels[i]==clusterI).all() and np.random.uniform()<0.3:

                ax = plt.subplot(n_clusters,12,1+count+12*clusterCount)
                tmp = np.clip(imgBatch_LR[i],0,255)
                ax.imshow(tmp[0],cmap='gray')
                ax.set_title("LR,{}".format(clusterI))
#                
##                ax = plt.subplot(n_clusters,12,count+7+12*clusterCount)
##                tmp = np.clip(imgBatch_HR[i],0,255)
##                ax.imshow(tmp[0],cmap='gray')
##                ax.set_title("HR {}".format(i))
#                
...
def checkComboCluster(imgBatch_LR,imgBatch_HR,labels):
    ...
    for global_clusterI in range(7):
        for local_clusterI in range(7):
            ...
            count = 0  
            for i in range(size):
                if labels[i,0]==global_clusterI and labels[i,1]==local_clusterI and np.random.uniform()<1:
...
trainset_LR_local = createLocalBlocks(trainset_LR[:5000])
#trainset_HR_local = createLocalBlocks(trainset_HR[:5000])
#_,trainset_LR_global = createGlobalSamples(trainset_LR[:5000])
...
cur_cluster_features_LR_local = trainset_LR_local
params_afterClassify_LR_local = {"n_keptComponent":opt.mapping_weight_thresh,"zoom factor":{'1':3},'cluster index':(None,None)}  
cur_cluster_features_LR_local= calcPCA_weight.calcW(cur_cluster_features_LR_local,params_afterClassify_LR_local,isbeforeCalssify=True,in_out_layers=['L0','L1'],\
...
#params_afterClassify_LR_global = {"n_keptComponent":opt.mapping_weight_thresh,"zoom factor":{'1':3,'2':3},'cluster index':(None,None)}  
#cur_cluster_features_LR_global= calcPCA_weight.calcW(cur_cluster_features_LR_global,params_afterClassify_LR_global,isbeforeCalssify=True,in_out_layers=['L0','L2'],\
#                                               mode = 'LR_scale_{}_interpo'.format(opt.down_scale),printPCAratio = opt.printPcaW)
...
#labels_global = meanShiftHelper(cur_cluster_features_LR_global.data.numpy(),n_clusters=2,meanShifts=None,n_features=3)
meanShifts_local,labels_local = meanShiftHelper(cur_cluster_features_LR_local.data.numpy(),n_clusters=None,meanShifts=None,n_features=2)
#overall_training_label = np.concatenate((labels_global,labels_local),acis = 1)

#checkCluster(meanShifts_local[1].cluster_centers_.shape[0],trainset_LR_local,trainset_HR_local,labels_local[:,[1]])
#checkCluster(opt.n_cluster_global,trainset_LR,trainset_HR,overall_training_label[:,3:])
#checkComboCluster(trainset_LR_local,trainset_LR_local,overall_training_label)

#meanShifts_local = PickleHelper('pickle',None,'/temp')
mylabels = np.expand_dims(meanShifts_local[0].labels_,axis = 1)
for i in range(1,5):
    mylabels = np.concatenate((mylabels,np.expand_dims(meanShifts_local[i].labels_,axis=1)),axis = 1)
iterTool,n_clusters = countHist(meanShifts_local,isPrint=False)
#tmp = np.concatenate((mylabels[:,[1]],mylabels[:,[3]]),axis = 1)
checkCluster(iterTool,n_clusters,trainset_LR_local,None,mylabels[:,1:])
